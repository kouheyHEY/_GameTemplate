...
- `docs/`: 開発ガイド、履歴、テンプレートなどのドキュメント。
    - `docs/common/`: 全プロジェクト共通のドキュメント（ガイド・テンプレート・将来計画）。
    - `docs/game/`: ゲーム開発に特化したドキュメント（知見・設計案）。
    - `docs/app/`: プロジェクト固有のドキュメント（履歴・ロードマップ）。
    - `docs/work/`: 作業用のメモ・下書き。
    - ※ 各ディレクトリには `plan/`（計画・設計）と `history/`（履歴・記録）を配置し、情報の性質に応じて整理します。
